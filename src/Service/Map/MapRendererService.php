...
class MapRendererService
{
    public function renderMap(string $file): array
    {
        ...
        foreach ($images as $z => $image) {
            $minimapFile = fopen("/var/www/html/public/img/minimaps/" . $mapname . "-" . $z . ".png", 'w');
            fwrite($minimapFile, base64_decode($image));
            fclose($minimapFile);
        }
        $mapSymbolFile = fopen("/var/www/html/public/img/minimaps/" . $mapname . ".symbols.json", 'w');
        $mapGridFile = fopen("/var/www/html/public/img/minimaps/" . $mapname . ".grid.json", 'w');
