...
class PNGMetadataExtractor
{
    ...
    public static function getMetadata($filename)
    {
        ...
        if ($buf != self::$pngSig) {
            throw new Exception(
                __METHOD__ . ": Not a valid PNG file; header: $buf"
            );
        }
        ...
        while (!feof($fh)) {
            ...
            if ($chunk_size < 0 || $chunk_size > self::MAX_CHUNK_SIZE) {
                ...
                ': Chunk size of ' .
                $chunk_size .
                ' too big, skipping. Max size is: ' .
                self::MAX_CHUNK_SIZE);
                if (
                    fseek($fh, 4 + $chunk_size + self::$crcSize, SEEK_CUR) !== 0
                ) {
            ...
            if ($chunk_type == 'IHDR') {
                    ...
                    if ($items[2] == "\x01") {
                        if (
                            ...
                            if ($items[5] === false) {
                                ...
                                wfDebug(
                                    __METHOD__ .
                                    ' Error decompressing iTxt chunk - ' .
                                    $items[1]
                                );
                                continue;
                            }
                ...
                if (function_exists('gzuncompress')) {
                    ...
                    if ($content === false) {
                        // decompression failed
                        wfDebug(__METHOD__ .
                            ' Error decompressing zTXt chunk - ' .
                            $keyword);
                        continue;
                    }
