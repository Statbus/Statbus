...
class GifCreator
{
    ...
    public function create($frames = array(), $durations = array(), $loop = 0)
    {
        if (!is_array($frames) && !is_array($durations)) {
            throw new \Exception(
                $this->version . ': ' . $this->errors['ERR00']
            );
        }
        ...
        for ($i = 0; $i < count($frames); $i++) {
            if (is_object($frames[$i]) || is_resource($frames[$i])) { // Resource var
                ...
                if (
                    substr($frames[$i], 0, 4) != 'GIF8' &&
                            file_exists($frames[$i]) ||
                        filter_var($frames[$i], FILTER_VALIDATE_URL)
                ) { // File path
                ...
                throw new \Exception(
                    $this->version . ': ' . $this->errors['ERR02']
                ); //.' ('.$mode.')'?
            }
            ...
            if (
                substr($this->frameSources[$i], 0, 6) != 'GIF87a' &&
                    substr($this->frameSources[$i], 0, 6) != 'GIF89a'
            ) {
                throw new \Exception(
                    $this->version . ': ' . $i . ' ' . $this->errors['ERR01']
                );
            }

            for (
                $j =
                    13 + (3 * (2 << (ord($this->frameSources[$i][10]) & 0x07))),
                    $k = true;
                $k;
                $j++
            ) {
                switch ($this->frameSources[$i][$j]) {
                    case '!':
                        if (
                            substr($this->frameSources[$i], $j + 3, 8) ==
                                'NETSCAPE'
                        ) {
                            throw new \Exception(
                                $this->version .
        ...
        if (ord($this->frameSources[0][10]) & 0x80) {
            ...
            if ($this->loop != 1) {
                ...
                $this->gif .=
                    ...
                    $this->encodeAsciiToChar($this->loop) .
                    "\0";
            }
        }
    }
    ...
    public function addGifFrames($i, $d)
    {
        ...
        $Global_rgb = substr(
            $this->frameSources[0],
            13,
            3 * (2 << (ord($this->frameSources[0][10]) & 0x07))
        );
        $Locals_rgb = substr(
            $this->frameSources[$i],
            13,
            3 * (2 << (ord($this->frameSources[$i][10]) & 0x07))
        );
        ...
        if ($this->colour > -1 && ord($this->frameSources[$i][10]) & 0x80) {
            for (
                $j = 0;
                $j < (2 << (ord($this->frameSources[$i][10]) & 0x07));
                $j++
            ) {
                if (
                    ord($Locals_rgb[(3 * $j) + 0]) ==
                            (($this->colour >> 16) & 0xFF) &&
                        ord($Locals_rgb[(3 * $j) + 1]) ==
                            (($this->colour >> 8) & 0xFF) &&
                        ord($Locals_rgb[(3 * $j) + 2]) ==
                            (($this->colour >> 0) & 0xFF)
                ) {
                    $Locals_ext =
                        "!\xF9\x04" .
        ...
        if (ord($this->frameSources[$i][10]) & 0x80 && $this->imgBuilt) {
            if ($Global_len == $Locals_len) {
                if (
                    ...
                    $this->gif .=
                        $Locals_ext . $Locals_img . $Locals_rgb . $Locals_tmp;
                }
            } else {
                $byte = ord($Locals_img[9]);
                $byte |= 0x80;
